...
void DelauneyTriangulation::delauneyTriangulation_Triangle()
{
	...
	m_vecPointsRef->push_back(osg::Vec3(0.0, 0.0, 0.0));
	m_vecPointsRef->push_back(osg::Vec3(1.0, 0.0, 0.0));
	m_vecPointsRef->push_back(osg::Vec3(1.0, 10.0, 0.0));
	m_vecPointsRef->push_back(osg::Vec3(0.0, 10.0, 0.0));

	osg::ref_ptr<osg::DrawElementsUInt> primitiveRef = new osg::DrawElementsUInt(osg::PrimitiveSet::TRIANGLES);

	for (int i = 0; i < in.numberofcorners; ++i)
	{
		primitiveRef->push_back(in.trianglelist[i]);
	}
