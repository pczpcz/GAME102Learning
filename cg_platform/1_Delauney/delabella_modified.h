...
extern uint64_t sorting_bench_modified;
...
struct IDelaBella2
{
	...
	struct Simplex
	{
		...
		bool IsDelaunay() const				//用于过滤所有的三角化面？？？？
		{
			return !(flags & 0b10000000);
		}

		bool IsInterior(int at) const		//能不能用于判断border？？？
		{
			return flags & 0b01000000;
		}

		bool IsEdgeFixed(int at) const		//可能和约束边有关？？？？
		{
			return flags & (1 << at);
		}
...
struct CDelaBella2 : IDelaBella2<T, I>
{
	...
	struct Face : IDelaBella2<T, I>::Simplex
	{
		...
		bool dotNP(const Vert &p) /*const*/
		{
			{ // somewhat faster, poor compiler inlining?
				...
				const T sum_abs_bdxcdy_cdxbdy = ABS(bdxcdy) + ABS(cdxbdy);
				...
				const T permanent = sum_abs_bdxcdy_cdxbdy * alift + (ABS(cdxady) + ABS(adxcdy)) * blift + (ABS(adxbdy) + ABS(bdxady)) * clift;
	...
	I Prepare(I *start, Face **hull, I *out_hull_faces, Face **cache, uint64_t *sort_stamp, I stop)
	{
		...
		// rmove dups
		{
			...
			sorting_bench_modified = time1 - *sort_stamp;
